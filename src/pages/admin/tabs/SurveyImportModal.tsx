...
import { parseSurveyExcel } from '../../../lib/surveyExcel'
import type { SurveyQuestionInput } from '../../../lib/surveyTypes'
import { SURVEY_TYPE_LABEL } from '../../../lib/surveyTypes'
...
export default function SurveyImportModal({ onClose, onImported }: Props) {
  const inputRef = useRef<HTMLInputElement>(null)
  const [fileName, setFileName] = useState<string | null>(null)
  const [rows, setRows] = useState<SurveyQuestionInput[]>([])
  ...
  async function handleFile(file: File) {
    ...
    try {
      const { rows, errors } = await parseSurveyExcel(file)
      setRows(rows)
      setErrors(errors)
    } catch (e) {
    ...
    >
      ...
      >
        <div className="px-6 pt-5 pb-3 border-b border-text-dark/10 flex items-center justify-between">
          <h2 className="text-lg font-bold text-text-dark">📥 설문 질문 일괄 업로드</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="닫기"
                      ...
                      <div className="overflow-x-auto rounded-xl border border-text-dark/10">
                        <table className="w-full text-xs">
                          <thead className="bg-cream">
                            <tr>
                              <th className="px-2 py-1.5 text-left font-bold">순서</th>
                              <th className="px-2 py-1.5 text-left font-bold">유형</th>
                              <th className="px-2 py-1.5 text-left font-bold">질문</th>
                              <th className="px-2 py-1.5 text-left font-bold">필수</th>
                            </tr>
                          </thead>
                          <tbody>
                            {rows.slice(0, 8).map((r, i) => (
                              <tr key={i} className="border-t border-text-dark/5">
                                ...
                                <td className="px-2 py-1.5">
                                  {SURVEY_TYPE_LABEL[r.question_type]}
                                </td>
