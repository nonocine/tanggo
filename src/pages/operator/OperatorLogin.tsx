...
const OPERATOR_PASSWORD_FALLBACK = 'onnastaff'
...
export default function OperatorLogin() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const expected =
      import.meta.env.VITE_OPERATOR_PASSWORD || OPERATOR_PASSWORD_FALLBACK
    if (password === expected) {
      localStorage.setItem(OPERATOR_AUTH_KEY, 'true')
    ...
    <div className="min-h-screen bg-cream">
      ...
      <main className="mx-auto max-w-md px-5 pt-10">
        <form
          ...
          className="relative rounded-3xl border-4 border-mint bg-white px-6 pt-10 pb-6"
        ...
        >
          ...
          <div className="text-center">
            ...
            <h2 className="mt-3 text-xl font-bold text-text-dark">운영자 로그인</h2>
            <p className="mt-1 text-sm text-text-dark/60">
              {SEASON_CONFIG.seasonName} 현장 운영
            </p>
          </div>
            ...
            <input
              ...
              className={`mt-2 w-full px-4 py-3.5 rounded-2xl border-2 bg-white text-base font-medium placeholder:text-text-dark/30 focus:outline-none focus:ring-2 transition-colors ${
                error
                  ...
                  : 'border-text-dark/10 focus:border-mint focus:ring-mint/30'
              }`}
            />
          ...
          <button
            ...
            className={`mt-2 w-full rounded-2xl py-4 text-lg font-bold transition-all duration-150 ${
              password.length > 0
                ? 'bg-mint text-text-dark hover:-translate-y-0.5 hover:bg-[#6FD491] active:translate-y-0 active:scale-[0.98]'
